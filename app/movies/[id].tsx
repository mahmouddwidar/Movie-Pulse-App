			...
			<ScrollView contentContainerStyle={{ paddingBottom: 80 }}>
				...
				<View className="flex-col justify-center items-start mt-5 px-5">
					...
					<View className="flex-row items-center mt-2 gap-x-1">
						...
						<Text className="font-bold text-light-100 mx-1">•</Text>
					...
					<View className="flex-row items-center bg-dark-200 px-2 py-1 rounded-md gap-x-1 mt-2">
						...
						<Text className="text-light-200 text-sm">
							...
							{movie?.vote_count > 1000
								? (movie?.vote_count / 1000).toFixed(
										movie?.vote_count % 1000 === 0 ? 0 : 1
								  )
								: movie?.vote_count}{" "}
					...
					<View className="flex flex-row justify-between w-1/2">
						...
						<MovieInfo
							label="Revenue"
							value={`$${(Math.round(movie?.revenue) / 1_000_000).toFixed(3)}`}
						/>
					</View>
